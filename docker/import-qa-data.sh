...
INDEX_NAME="tibetan_qa"

echo "========================================="
echo "Tibetan QA Data Import Script"
echo "========================================="
...
# Create index with QA-specific mappings
...
echo "Importing QA data from /tibetan-qa-data.json..."
BULK_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$ES_URL/$INDEX_NAME/_bulk" \
  -H 'Content-Type: application/x-ndjson' \
  --data-binary @/tibetan-qa-data.json)
...
echo ""
echo "Index settings:"
curl -s "$ES_URL/$INDEX_NAME/_settings" | grep -A 3 '"number_of_shards"'
...
echo ""
echo "Test queries:"
echo "  curl \"http://localhost:9200/$INDEX_NAME/_search?q=title:རྟ་རྩལ&size=1\""
echo "  curl \"http://localhost:9200/$INDEX_NAME/_search?q=question:ལོ་རྒྱུས&size=5\""
echo ""
